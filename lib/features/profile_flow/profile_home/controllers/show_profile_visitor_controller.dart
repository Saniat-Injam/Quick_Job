...
import 'package:quick_job/features/profile_flow/profile_home/models/get_profile_visitor_list_model.dart';

class ShowProfileVisitorController extends GetxController {
  // for pagination
  final scrollController = ScrollController();
  @override
  void onInit() {
    super.onInit();
    getProfileVisitorList(isRefresh: true);
  ...
  int limit = 20;
  bool hasMore = true;
  final profileVisitor = <ProfileVisitorInfo>[].obs;
  // for get user all post
  Future<void> getProfileVisitorList({bool isRefresh = false}) async {
    try {
      if (isRefresh) {
        currantPage = 1;
        hasMore = true;
        profileVisitor.clear();
      }
      if (!hasMore) return;
      if (currantPage == 1) {
        isUserLoading(true);
      ...
      final response = await NetworkCaller().getRequest(
        "${AppUrls.getProfileVisitorList}?limit=$limit&page=$currantPage",
        // token: AuthService.token.toString(),
      );
      if (response.isSuccess) {
        log("post fetch successful! for $currantPage");
        final data = GetProfileVisitionUserListModel.fromJson(response.responseData);
        if (data.result?.data != null && (data.result?.data ?? []).isNotEmpty) {
          profileVisitor.addAll(data.result?.data ?? []);
