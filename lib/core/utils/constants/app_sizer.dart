...
extension ResponsiveExtension on num {
  ...
  // sp stands for “scaled pixels”
  // it’s used specifically for font sizes to make text responsive
  // across different screen sizes and pixel densities.

  double get sp => (this * SizeUtils.width) / kFigmaDesignWidth;

  /// Radius / Border size responsive
  double get r => (this * SizeUtils.radius) / kFigmaDesignHeight;
}
...
class SizeUtils {
  ...
  static void setScreenSize(
    ...
    if (orientation == Orientation.portrait) {
      width = boxConstraints.maxWidth.nonZero(defaultValue: kFigmaDesignWidth);
      height = boxConstraints.maxHeight.nonZero(
        defaultValue: kFigmaDesignHeight,
      );
    } else {
      width = boxConstraints.maxHeight.nonZero(defaultValue: kFigmaDesignWidth);
      height = boxConstraints.maxWidth.nonZero(
    ...
    // Define radius based on average of width & height (for consistent scaling)
    radius = ((width + height) / 2).nonZero(defaultValue: kFigmaDesignHeight);
